...
public class ResultPopupController : MonoBehaviour
{
    ...
    private void DisplayRewards(StageResultData resultData)
    {
        ...
        if (rewardItemsContainer != null && itemSlotPrefab != null)
        {
            ...
            // 아이템이 없으면 컨테이너 숨김
            if (resultData.itemRewards == null || resultData.itemRewards.Count == 0)
            {
                if (enableDebugLogs)
                {
            ...
            // 아이템 개수에 따라 레이아웃 조정
            AdjustItemLayout(resultData.itemRewards.Count);
            ...
            if (enableDebugLogs)
            {
                Debug.Log($"📦 [ResultPopupController] 아이템 슬롯 생성 완료: {resultData.itemRewards.Count}개");
            }
        }
        else
        {
