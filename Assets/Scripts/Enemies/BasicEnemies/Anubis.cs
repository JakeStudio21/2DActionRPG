...
/// <summary>
/// Anubis 몬스터 클래스 - 근접 공격 특화 (8방향 스프라이트)
/// ⭐ [Complete Reset] 완전한 데이터 기반 시스템
/// </summary>
public class Anubis : BaseEnemy
{
    [Header("⭐ Anubis 전용 컴포넌트")]
    ...
    { 
        get 
        {
            ...
            // 데이터 기반 fallback (Anubis는 중간 패트롤 범위)
            if (enemyData != null)
    ...
    { 
        get 
        {
            ...
            Debug.LogError($"[Anubis] {gameObject.name}: MeleeAttack 또는 AttackData가 없습니다!");
            return 1.3f; // Anubis는 긴 공격 범위 (창 공격)
        } 
    }
    ...
    public override float DetectionRange
    {
        get
        {
            if (EnemyData != null)
            {
                ...
                Debug.Log($"[Anubis] {gameObject.name} DetectionRange: {range} (from EnemyData)");
                return range;
            }
                
            Debug.LogError($"[Anubis] {gameObject.name}: EnemyData가 없어서 DetectionRange 확인 불가! fallback 6f 사용");
            return 6f; // Anubis는 넓은 감지 범위
        }
    }
    ...
    { 
        get 
        {
            if (EnemyData != null)
            {
                ...
                Debug.Log($"[Anubis] {gameObject.name} ChaseRange: {range} (from EnemyData)");
                return range;
            }
                
            Debug.LogError($"[Anubis] {gameObject.name}: EnemyData가 없어서 ChaseRange 확인 불가! fallback 8f 사용");
            return 8f; // Anubis는 긴 추적 범위
        } 
    }
    ...
    protected override void OnAwakeInitialize()
    {
        ...
        Debug.Log($"[Anubis] {gameObject.name} Awake 초기화 완료");
    }

    protected override void OnStartInitialize()
    {
        ApplyAnubisSpecificSettings();
        Debug.Log($"[Anubis] {gameObject.name} Start 초기화 완료");
    }
    ...
    protected override void InitializeAttackSystem()
    {
        if (meleeAttack != null)
        {
            ...
            Debug.Log($"[Anubis] {gameObject.name} MeleeAttack 시스템 초기화 완료");
        }
        else
        {
            Debug.LogError($"[Anubis] {gameObject.name}: MeleeAttack 컴포넌트가 없습니다!");
        }
    }
    ...
    public override void Attack()
    {
        ...
        {
            meleeAttack.Attack();
            Debug.Log($"[Anubis] {gameObject.name} 근접 공격 실행!");
        }
    }
    ...
    /// Anubis 전용 설정 적용
    /// </summary>
    private void ApplyAnubisSpecificSettings()
    {
        if (EnemyData != null)
        {
            Debug.Log($"[Anubis] {gameObject.name} 전용 설정 적용:");
    ...
    private void ForceEliteType()
    {
        if (EnemyData != null)
        {
            Debug.Log($"[Anubis] Elite 타입 적용 - 새 체력: {GetScaledMaxHealth():F1}");
            Debug.Log($"[Anubis] 공격력은 AttackData에서 관리됨");
        }
    }
    ...
    private void TestMeleeDamage()
    {
        if (meleeAttack != null)
        {
            ...
            Debug.Log($"[Anubis] 실제 근접 데미지: {actualDamage}");
        }
        else
        {
            Debug.LogError($"[Anubis] MeleeAttack 컴포넌트가 없어서 데미지 테스트 불가!");
        }
    }
    ...
    public override void LevelUp()
    {
        ...
        Debug.Log($"[Anubis] {gameObject.name} 레벨업! 새 레벨: {CurrentLevel}");
        Debug.Log($"[Anubis] 새 체력: {GetScaledMaxHealth():F1}");
        Debug.Log($"[Anubis] 공격력은 AttackData에서 관리됨");
    }
    ...
    /// Anubis 상태 디버그 출력
    /// </summary>
    [ContextMenu("Debug Anubis Info")]
    private void DebugAnubisInfo()
    {
        string info = $"=== Anubis {gameObject.name} ===\n";
