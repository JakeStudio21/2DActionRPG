...
public class LadyBug : BaseEnemy
{
    ...
    { 
        get 
        {
            ...
            // 데이터 기반 fallback (LadyBug는 넓은 패트롤 범위)
            if (enemyData != null)
                return enemyData.PatrolRadius;
            
            return 5f; // 기본값 (활동적인 순찰)
        } 
    }
    ...
    { 
        get 
        {
            ...
            Debug.LogError($"[LadyBug] {gameObject.name}: MeleeAttack 또는 AttackData가 없습니다!");
            return 0.9f; // LadyBug는 짧은 공격 범위
        } 
    }
    ...
    public override float DetectionRange
    {
        get
        {
            if (EnemyData != null)
            {
                ...
                Debug.Log($"[LadyBug] {gameObject.name} DetectionRange: {range} (from EnemyData)");
                return range;
            }
                
            Debug.LogError($"[LadyBug] {gameObject.name}: EnemyData가 없어서 DetectionRange 확인 불가! fallback 6.5f 사용");
            return 6.5f; // LadyBug는 넓은 감지 범위
        }
    }
    ...
    { 
        get 
        {
            if (EnemyData != null)
            {
                ...
                Debug.Log($"[LadyBug] {gameObject.name} ChaseRange: {range} (from EnemyData)");
                return range;
            }
                
            Debug.LogError($"[LadyBug] {gameObject.name}: EnemyData가 없어서 ChaseRange 확인 불가! fallback 9f 사용");
            return 9f; // LadyBug는 긴 추적 범위 (끈질긴 추적)
        } 
    }
    ...
    protected override void OnAwakeInitialize()
    {
        ...
        Debug.Log($"[LadyBug] {gameObject.name} Awake 초기화 완료");
    }

    protected override void OnStartInitialize()
    {
        ApplyLadyBugSpecificSettings();
        Debug.Log($"[LadyBug] {gameObject.name} Start 초기화 완료");
    }
    ...
    protected override void InitializeAttackSystem()
    {
        if (meleeAttack != null)
        {
            ...
            Debug.Log($"[LadyBug] {gameObject.name} MeleeAttack 시스템 초기화 완료");
        }
        else
        {
            Debug.LogError($"[LadyBug] {gameObject.name}: MeleeAttack 컴포넌트가 없습니다!");
        }
    }
    ...
    public override void Attack()
    {
        ...
        {
            meleeAttack.Attack();
            Debug.Log($"[LadyBug] {gameObject.name} 근접 공격 실행!");
        }
    }

    #endregion

    #region ⭐ LadyBug 전용 설정

    /// <summary>
    /// LadyBug 전용 설정 적용
    /// </summary>
    private void ApplyLadyBugSpecificSettings()
    {
        if (EnemyData != null)
        {
            Debug.Log($"[LadyBug] {gameObject.name} 전용 설정 적용:");
            ...
            Debug.Log($"  - 특성: 매우 빠른 이동속도, 낮은 방어력, 끈질긴 추적");
        }
    }
    ...
    private void ForceEliteType()
    {
        if (EnemyData != null)
        {
            Debug.Log($"[LadyBug] Elite 타입 적용 - 새 체력: {GetScaledMaxHealth():F1}");
            Debug.Log($"[LadyBug] 공격력은 AttackData에서 관리됨");
        }
    }
    ...
    private void TestMeleeDamage()
    {
        if (meleeAttack != null)
        {
            ...
            Debug.Log($"[LadyBug] 실제 근접 데미지: {actualDamage}");
        }
        else
        {
            Debug.LogError($"[LadyBug] MeleeAttack 컴포넌트가 없어서 데미지 테스트 불가!");
        }
    }
    ...
    public override void LevelUp()
    {
        ...
        Debug.Log($"[LadyBug] {gameObject.name} 레벨업! 새 레벨: {CurrentLevel}");
        Debug.Log($"[LadyBug] 새 체력: {GetScaledMaxHealth():F1}");
        Debug.Log($"[LadyBug] 공격력은 AttackData에서 관리됨");
    }
    ...
    /// LadyBug 상태 디버그 출력
    /// </summary>
    [ContextMenu("Debug LadyBug Info")]
    private void DebugLadyBugInfo()
    {
        string info = $"=== LadyBug {gameObject.name} ===\n";
