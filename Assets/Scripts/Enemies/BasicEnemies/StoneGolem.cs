...
/// <summary>
/// StoneGolem 몬스터 클래스 - 근접 공격 특화 (BlueSlime 기반)
/// ⭐ [Complete Reset] 완전한 데이터 기반 시스템
/// </summary>
public class StoneGolem : BaseEnemy
{
    [Header("⭐ StoneGolem 전용 컴포넌트")]
    ...
    { 
        get 
        {
            ...
            // 데이터 기반 fallback (StoneGolem은 더 작은 패트롤 범위)
            if (enemyData != null)
                return enemyData.PatrolRadius;
            
            return 2f; // 기본값 (BlueSlime 3f보다 작음)
        } 
    }
    ...
    { 
        get 
        {
            ...
            Debug.LogError($"[StoneGolem] {gameObject.name}: MeleeAttack 또는 AttackData가 없습니다!");
            return 1.2f; // StoneGolem은 더 긴 공격 범위
        } 
    }
    ...
    public override float DetectionRange
    {
        get
        {
            ...
            Debug.LogError($"[StoneGolem] {gameObject.name}: EnemyData가 없어서 DetectionRange 확인 불가!");
            return 5f; // StoneGolem은 기본 감지 범위
        }
    }
    ...
    { 
        get 
        {
            ...
            Debug.LogError($"[StoneGolem] {gameObject.name}: EnemyData가 없어서 ChaseRange 확인 불가!");
            return 6f; // StoneGolem은 더 짧은 추적 범위
        } 
    }
    ...
    protected override void OnAwakeInitialize()
    {
        ...
        Debug.Log($"[StoneGolem] {gameObject.name} Awake 초기화 완료");
    }

    protected override void OnStartInitialize()
    {
        ApplyStoneGolemSpecificSettings();
        Debug.Log($"[StoneGolem] {gameObject.name} Start 초기화 완료");
    }
    ...
    protected override void InitializeAttackSystem()
    {
        if (meleeAttack != null)
        {
            ...
            Debug.Log($"[StoneGolem] {gameObject.name} MeleeAttack 시스템 초기화 완료");
        }
        else
        {
            Debug.LogError($"[StoneGolem] {gameObject.name}: MeleeAttack 컴포넌트가 없습니다!");
        }
    }
    ...
    public override void Attack()
    {
        ...
        {
            meleeAttack.Attack();
            Debug.Log($"[StoneGolem] {gameObject.name} 근접 공격 실행!");
        }
    }

    #endregion

    #region ⭐ StoneGolem 전용 설정

    /// <summary>
    /// StoneGolem 전용 설정 적용
    /// </summary>
    private void ApplyStoneGolemSpecificSettings()
    {
        if (EnemyData != null)
        {
            Debug.Log($"[StoneGolem] {gameObject.name} 전용 설정 적용:");
            ...
            Debug.Log($"  - 특성: 느린 이동속도, 높은 방어력, 긴 공격범위");
        }
    }
    ...
    private void ForceEliteType()
    {
        if (EnemyData != null)
        {
            Debug.Log($"[StoneGolem] Elite 타입 적용 - 새 체력: {GetScaledMaxHealth():F1}");
            Debug.Log($"[StoneGolem] 공격력은 AttackData에서 관리됨");
        }
    }
    ...
    private void TestMeleeDamage()
    {
        if (meleeAttack != null)
        {
            ...
            Debug.Log($"[StoneGolem] 실제 근접 데미지: {actualDamage}");
        }
        else
        {
            Debug.LogError($"[StoneGolem] MeleeAttack 컴포넌트가 없어서 데미지 테스트 불가!");
        }
    }
    ...
    public override void LevelUp()
    {
        ...
        Debug.Log($"[StoneGolem] {gameObject.name} 레벨업! 새 레벨: {CurrentLevel}");
        Debug.Log($"[StoneGolem] 새 체력: {GetScaledMaxHealth():F1}");
        Debug.Log($"[StoneGolem] 공격력은 AttackData에서 관리됨");
    }
    ...
    /// <summary>
    /// StoneGolem 상태 디버그 출력
    /// </summary>
    [ContextMenu("Debug StoneGolem Info")]
    private void DebugStoneGolemInfo()
    {
        string info = $"=== StoneGolem {gameObject.name} ===\n";
