...
/// <summary>
/// WaterGolem 몬스터 클래스 - AOE 공격 특화
/// ⭐ [New System] AOE 공격 시스템 기반 몬스터
/// </summary>
public class WaterGolem : BaseEnemy
{
    [Header("⭐ WaterGolem 전용 컴포넌트")]
    ...
    { 
        get 
        {
            ...
            // 데이터 기반 fallback (WaterGolem은 중간 패트롤 범위)
            if (enemyData != null)
    ...
    { 
        get 
        {
            ...
            Debug.LogError($"[WaterGolem] {gameObject.name}: AOEAttack 또는 AttackData가 없습니다!");
            return 4f; // WaterGolem은 AOE 공격으로 더 긴 범위
        } 
    }
    ...
    public override void Attack()
    {
        if (aoeAttack != null)
        {
            Debug.Log($"[WaterGolem] {gameObject.name} - AOE 공격 실행!");
            aoeAttack.Attack();
        }
        else
        {
            Debug.LogError($"[WaterGolem] {gameObject.name}: AOEAttack 컴포넌트가 없습니다!");
        }
    }

    protected override void InitializeAttackSystem()
    {
        Debug.Log($"[WaterGolem] {gameObject.name} - AOE 공격 시스템 초기화");
        
        // AOEAttack 컴포넌트 초기화
        if (aoeAttack != null)
        {
            aoeAttack.Initialize();
            Debug.Log($"[WaterGolem] AOEAttack 초기화 완료");
        }
        else
        {
            Debug.LogError($"[WaterGolem] {gameObject.name}: AOEAttack 컴포넌트가 없습니다!");
    ...
    {
        Debug.Log($"[WaterGolem] {gameObject.name} - Awake 초기화");
    ...
    {
        Debug.Log($"[WaterGolem] {gameObject.name} - Start 초기화");
        
        // WaterGolem 전용 초기화
        InitializeWaterGolem();
    }
    ...
    #region WaterGolem 전용 초기화

    /// <summary>
    /// WaterGolem 전용 초기화
    /// </summary>
    private void InitializeWaterGolem()
    {
        Debug.Log($"[WaterGolem] {gameObject.name} - WaterGolem 초기화 시작");
        
        // AOEAttack 컴포넌트 검증
        ValidateAOEAttackComponent();
        
        // WaterGolem 전용 설정
        ConfigureWaterGolemSettings();
        
        Debug.Log($"[WaterGolem] {gameObject.name} - WaterGolem 초기화 완료");
    }
    ...
    private void ValidateAOEAttackComponent()
    {
        if (aoeAttack == null)
        {
            Debug.LogError($"[WaterGolem] {gameObject.name}: AOEAttack 컴포넌트가 할당되지 않았습니다!");
        ...
        {
            Debug.LogError($"[WaterGolem] {gameObject.name}: AOEAttack의 AttackData가 설정되지 않았습니다!");
        ...
        {
            Debug.LogWarning($"[WaterGolem] {gameObject.name}: AttackData의 공격 타입이 AOE가 아닙니다: {aoeAttack.AttackData.AttackType}");
        }

        Debug.Log($"[WaterGolem] AOEAttack 컴포넌트 검증 완료");
    }

    /// <summary>
    /// WaterGolem 전용 설정
    /// </summary>
    private void ConfigureWaterGolemSettings()
    {
        // WaterGolem은 느리지만 강력한 몬스터
        if (enemyData != null)
        {
            Debug.Log($"[WaterGolem] EnemyData 기반 설정:");
        ...
        if (aoeAttack != null && aoeAttack.AttackData != null)
        {
            Debug.Log($"[WaterGolem] AOE 공격 설정:");
    ...
    /// WaterGolem 디버그 정보
    /// </summary>
    [ContextMenu("Debug WaterGolem Info")]
    public void DebugWaterGolemInfo()
    {
        string info = $"=== WaterGolem {gameObject.name} ===\n";
    ...
    public void TestAOEAttack()
    {
        if (aoeAttack != null)
        {
            Debug.Log($"[WaterGolem] {gameObject.name} - AOE 공격 테스트 실행!");
            aoeAttack.Attack();
        }
        else
        {
