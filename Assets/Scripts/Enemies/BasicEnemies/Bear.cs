...
/// <summary>
/// Bear 몬스터 클래스 - 근접 공격 특화 (8방향 스프라이트)
...
public class Bear : BaseEnemy
{
    ...
    { 
        get 
        {
            ...
            // 데이터 기반 fallback (Bear는 중간 패트롤 범위)
            if (enemyData != null)
    ...
    { 
        get 
        {
            ...
            Debug.LogError($"[Bear] {gameObject.name}: MeleeAttack 또는 AttackData가 없습니다!");
            return 1.3f; // Bear는 긴 공격 범위 (강력한 할퀴기)
        } 
    }
    ...
    protected override void OnAwakeInitialize()
    {
        ...
        Debug.Log($"[Bear] {gameObject.name} Awake 초기화 완료");
    }

    protected override void OnStartInitialize()
    {
        ApplyBearSpecificSettings();
        Debug.Log($"[Bear] {gameObject.name} Start 초기화 완료");
    }
    ...
    protected override void InitializeAttackSystem()
    {
        if (meleeAttack != null)
        {
            ...
            Debug.Log($"[Bear] {gameObject.name} MeleeAttack 시스템 초기화 완료");
        }
        else
        {
            Debug.LogError($"[Bear] {gameObject.name}: MeleeAttack 컴포넌트가 없습니다!");
        }
    }
    ...
    public override void Attack()
    {
        ...
        {
            meleeAttack.Attack();
            Debug.Log($"[Bear] {gameObject.name} 근접 공격 실행!");
        }
    }
    ...
    /// Bear 전용 설정 적용
    /// </summary>
    private void ApplyBearSpecificSettings()
    {
        if (EnemyData != null)
        {
            Debug.Log($"[Bear] {gameObject.name} 전용 설정 적용:");
            ...
            Debug.Log($"  - 특성: 높은 체력, 강력한 데미지, 긴 공격 범위");
        }
    }
    ...
    private void ForceEliteType()
    {
        if (EnemyData != null)
        {
            Debug.Log($"[Bear] Elite 타입 적용 - 새 체력: {GetScaledMaxHealth():F1}");
            Debug.Log($"[Bear] 공격력은 AttackData에서 관리됨");
        }
    }
    ...
    private void TestMeleeDamage()
    {
        if (meleeAttack != null)
        {
            ...
            Debug.Log($"[Bear] 실제 근접 데미지: {actualDamage}");
        }
        else
        {
            Debug.LogError($"[Bear] MeleeAttack 컴포넌트가 없어서 데미지 테스트 불가!");
        }
    }
    ...
    public override void LevelUp()
    {
        ...
        Debug.Log($"[Bear] {gameObject.name} 레벨업! 새 레벨: {CurrentLevel}");
        Debug.Log($"[Bear] 새 체력: {GetScaledMaxHealth():F1}");
        Debug.Log($"[Bear] 공격력은 AttackData에서 관리됨");
    }
    ...
    /// Bear 상태 디버그 출력
    /// </summary>
    [ContextMenu("Debug Bear Info")]
    private void DebugBearInfo()
    {
        string info = $"=== Bear {gameObject.name} ===\n";
