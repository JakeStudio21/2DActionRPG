...
/// <summary>
/// Sphinx 몬스터 클래스 - 근접 공격 특화 (8방향 스프라이트)
/// ⭐ [Complete Reset] 완전한 데이터 기반 시스템
/// </summary>
public class Sphinx : BaseEnemy
{
    [Header("⭐ Sphinx 전용 컴포넌트")]
    ...
    { 
        get 
        {
            ...
            // 데이터 기반 fallback (Sphinx는 중간 패트롤 범위)
            if (enemyData != null)
    ...
    { 
        get 
        {
            ...
            Debug.LogError($"[Sphinx] {gameObject.name}: MeleeAttack 또는 AttackData가 없습니다!");
            return 1.3f; // Sphinx는 긴 공격 범위 (발톱 공격)
        } 
    }
    ...
    public override float DetectionRange
    {
        get
        {
            if (EnemyData != null)
            {
                ...
                Debug.Log($"[Sphinx] {gameObject.name} DetectionRange: {range} (from EnemyData)");
                return range;
            }
                
            Debug.LogError($"[Sphinx] {gameObject.name}: EnemyData가 없어서 DetectionRange 확인 불가! fallback 5.5f 사용");
            return 5.5f; // Sphinx는 보통 감지 범위
        }
    }
    ...
    { 
        get 
        {
            if (EnemyData != null)
            {
                ...
                Debug.Log($"[Sphinx] {gameObject.name} ChaseRange: {range} (from EnemyData)");
                return range;
            }
                
            Debug.LogError($"[Sphinx] {gameObject.name}: EnemyData가 없어서 ChaseRange 확인 불가! fallback 7.5f 사용");
            return 7.5f; // Sphinx는 중간 추적 범위
        } 
    }
    ...
    protected override void OnAwakeInitialize()
    {
        ...
        Debug.Log($"[Sphinx] {gameObject.name} Awake 초기화 완료");
    }

    protected override void OnStartInitialize()
    {
        ApplySphinxSpecificSettings();
        Debug.Log($"[Sphinx] {gameObject.name} Start 초기화 완료");
    }
    ...
    protected override void InitializeAttackSystem()
    {
        if (meleeAttack != null)
        {
            ...
            Debug.Log($"[Sphinx] {gameObject.name} MeleeAttack 시스템 초기화 완료");
        }
        else
        {
            Debug.LogError($"[Sphinx] {gameObject.name}: MeleeAttack 컴포넌트가 없습니다!");
        }
    }
    ...
    public override void Attack()
    {
        ...
        {
            meleeAttack.Attack();
            Debug.Log($"[Sphinx] {gameObject.name} 근접 공격 실행!");
        }
    }
    ...
    /// Sphinx 전용 설정 적용
    /// </summary>
    private void ApplySphinxSpecificSettings()
    {
        if (EnemyData != null)
        {
            Debug.Log($"[Sphinx] {gameObject.name} 전용 설정 적용:");
            ...
            Debug.Log($"  - 특성: 균형잡힌 스탯, 강력한 발톱 공격");
        }
    }
    ...
    private void ForceEliteType()
    {
        if (EnemyData != null)
        {
            Debug.Log($"[Sphinx] Elite 타입 적용 - 새 체력: {GetScaledMaxHealth():F1}");
            Debug.Log($"[Sphinx] 공격력은 AttackData에서 관리됨");
        }
    }
    ...
    private void TestMeleeDamage()
    {
        if (meleeAttack != null)
        {
            ...
            Debug.Log($"[Sphinx] 실제 근접 데미지: {actualDamage}");
        }
        else
        {
            Debug.LogError($"[Sphinx] MeleeAttack 컴포넌트가 없어서 데미지 테스트 불가!");
        }
    }
    ...
    public override void LevelUp()
    {
        ...
        Debug.Log($"[Sphinx] {gameObject.name} 레벨업! 새 레벨: {CurrentLevel}");
        Debug.Log($"[Sphinx] 새 체력: {GetScaledMaxHealth():F1}");
        Debug.Log($"[Sphinx] 공격력은 AttackData에서 관리됨");
    }
    ...
    /// Sphinx 상태 디버그 출력
    /// </summary>
    [ContextMenu("Debug Sphinx Info")]
    private void DebugSphinxInfo()
    {
        string info = $"=== Sphinx {gameObject.name} ===\n";
