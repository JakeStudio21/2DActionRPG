...
/// <summary>
/// Elite_Boar 엘리트 몬스터 클래스 - 근접 공격 + 스킬 (8방향 스프라이트)
/// ⭐ 엘리트 전용: 평타 60% + 스킬 40% 확률 시스템
/// ⭐ [Complete Reset] 완전한 데이터 기반 시스템
/// </summary>
public class Elite_Boar : BaseEnemy
{
    [Header("⭐ Elite_Boar 전용 컴포넌트")]
    ...
    { 
        get 
        {
            ...
            // 데이터 기반 fallback (Boar는 넓은 패트롤 범위)
            if (enemyData != null)
    ...
    protected override void OnAwakeInitialize()
    {
        ...
        Debug.Log($"[Elite_Boar] {gameObject.name} Awake 초기화 완료");
    }

    protected override void OnStartInitialize()
    {
        ApplyEliteBoarSpecificSettings();
        Debug.Log($"[Elite_Boar] {gameObject.name} Start 초기화 완료");
    }
    ...
    protected override void InitializeAttackSystem()
    {
        ...
        if (meleeAttack != null)
        {
            ...
            Debug.Log($"[Elite_Boar] {gameObject.name} MeleeAttack 시스템 초기화 완료");
        }
        else
        {
            Debug.LogError($"[Elite_Boar] {gameObject.name}: MeleeAttack 컴포넌트가 없습니다!");
        }
        ...
        if (eliteAttack != null)
        {
            ...
            Debug.Log($"[Elite_Boar] {gameObject.name} EliteAttackBehaviour 시스템 초기화 완료");
        }
        else
        {
            Debug.LogError($"[Elite_Boar] {gameObject.name}: EliteAttackBehaviour 컴포넌트가 없습니다!");
        }
    }
    ...
    public override void Attack()
    {
        ...
        {
            eliteAttack.Attack();
            Debug.Log($"[Elite_Boar] {gameObject.name} 엘리트 공격 실행 (평타 or 스킬)!");
        }
        else if (meleeAttack != null && meleeAttack.CanAttack())
        {
            // Fallback: EliteAttackBehaviour가 없으면 평타만
            meleeAttack.Attack();
            Debug.LogWarning($"[Elite_Boar] {gameObject.name} EliteAttackBehaviour 없음 - 평타로 fallback");
        }
    }

    #endregion

    #region ⭐ Elite_Boar 전용 설정

    /// <summary>
    /// Elite_Boar 전용 설정 적용
    /// </summary>
    private void ApplyEliteBoarSpecificSettings()
    {
        if (EnemyData != null)
        {
            Debug.Log($"[Elite_Boar] {gameObject.name} 전용 설정 적용:");
            ...
            Debug.Log($"  - 특성: 엘리트 멧돼지, 평타 60% + 스킬 40%");
    ...
    private void VerifyEliteType()
    {
        if (EnemyData != null)
        {
            Debug.Log($"[Elite_Boar] 타입 확인: {EnemyData.EnemyType}");
            Debug.Log($"[Elite_Boar] Elite 체력: {GetScaledMaxHealth():F1}");
            Debug.Log($"[Elite_Boar] 평타 확률: {EnemyData.MeleeAttackProbability}%");
            Debug.Log($"[Elite_Boar] 스킬 확률: {EnemyData.SkillUseProbability}%");
        }
    }
    ...
    private void TestMeleeDamage()
    {
        if (meleeAttack != null)
        {
            ...
            Debug.Log($"[Elite_Boar] 실제 근접 데미지: {actualDamage}");
        }
        else
        {
            Debug.LogError($"[Elite_Boar] MeleeAttack 컴포넌트가 없어서 데미지 테스트 불가!");
        }
    }
    ...
    private void CheckSkillCooldowns()
    {
        ...
        {
            Debug.LogWarning($"[Elite_Boar] EliteSkillController가 없습니다!");
        }
    }
    ...
    private void CheckAttackStatistics()
    {
        ...
        {
            Debug.LogWarning($"[Elite_Boar] EliteAttackBehaviour가 없습니다!");
        }
    }
    ...
    public override void LevelUp()
    {
        ...
        Debug.Log($"[Elite_Boar] {gameObject.name} 레벨업! 새 레벨: {CurrentLevel}");
        Debug.Log($"[Elite_Boar] 새 체력: {GetScaledMaxHealth():F1}");
        Debug.Log($"[Elite_Boar] 공격력은 AttackData에서 관리됨");
    }
    ...
    /// <summary>
    /// Elite_Boar 상태 디버그 출력
    /// </summary>
    [ContextMenu("Debug Elite_Boar Info")]
    private void DebugEliteBoarInfo()
    {
        string info = $"=== Elite_Boar {gameObject.name} ===\n";
    ...
    private void OnValidate()
    {
        ...
        {
            Debug.LogWarning($"[Elite_Boar] {name}의 아이소메트릭 데이터가 유효하지 않습니다.");
            isometricData.SetDefaults();
        }
    }
