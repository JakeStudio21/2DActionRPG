...
/// <summary>
/// Elite_Orc 엘리트 몬스터 클래스 - 근접 공격 + 스킬 (8방향 스프라이트)
/// ⭐ 엘리트 전용: 평타 60% + 스킬 40% 확률 시스템
/// ⭐ [Complete Reset] 완전한 데이터 기반 시스템
/// </summary>
public class Elite_Orc : BaseEnemy
{
    [Header("⭐ Elite_Orc 전용 컴포넌트")]
    ...
    { 
        get 
        {
            ...
            // 데이터 기반 fallback (Orc는 넓은 패트롤 범위)
            if (enemyData != null)
                return enemyData.PatrolRadius;
            
            return 4f; // 기본값 (공격적인 순찰)
        } 
    }
    ...
    { 
        get 
        {
            ...
            Debug.LogError($"[Elite_Orc] {gameObject.name}: MeleeAttack 또는 AttackData가 없습니다!");
            return 1.8f; // Orc는 긴 팔 공격 범위
        } 
    }
    ...
    public override float DetectionRange
    {
        get
        {
            if (EnemyData != null)
            {
                ...
                Debug.Log($"[Elite_Orc] {gameObject.name} DetectionRange: {range} (from EnemyData)");
                return range;
            }
                
            Debug.LogError($"[Elite_Orc] {gameObject.name}: EnemyData가 없어서 DetectionRange 확인 불가! fallback 7f 사용");
    ...
    { 
        get 
        {
            if (EnemyData != null)
            {
                ...
                Debug.Log($"[Elite_Orc] {gameObject.name} ChaseRange: {range} (from EnemyData)");
                return range;
            }
                
            Debug.LogError($"[Elite_Orc] {gameObject.name}: EnemyData가 없어서 ChaseRange 확인 불가! fallback 9f 사용");
            return 9f; // Elite는 긴 추적 범위 (끈질긴 추격)
        } 
    }
    ...
    protected override void OnAwakeInitialize()
    {
        ...
        Debug.Log($"[Elite_Orc] {gameObject.name} Awake 초기화 완료");
    }

    protected override void OnStartInitialize()
    {
        ApplyEliteOrcSpecificSettings();
        Debug.Log($"[Elite_Orc] {gameObject.name} Start 초기화 완료");
    }
    ...
    protected override void InitializeAttackSystem()
    {
        ...
        if (meleeAttack != null)
        {
            ...
            Debug.Log($"[Elite_Orc] {gameObject.name} MeleeAttack 시스템 초기화 완료");
        }
        else
        {
            Debug.LogError($"[Elite_Orc] {gameObject.name}: MeleeAttack 컴포넌트가 없습니다!");
        }
        ...
        if (eliteAttack != null)
        {
            ...
            Debug.Log($"[Elite_Orc] {gameObject.name} EliteAttackBehaviour 시스템 초기화 완료");
        }
        else
        {
            Debug.LogError($"[Elite_Orc] {gameObject.name}: EliteAttackBehaviour 컴포넌트가 없습니다!");
        }
    }
    ...
    public override void Attack()
    {
        ...
        {
            eliteAttack.Attack();
            Debug.Log($"[Elite_Orc] {gameObject.name} 엘리트 공격 실행 (평타 or 스킬)!");
        }
        else if (meleeAttack != null && meleeAttack.CanAttack())
        {
            // Fallback: EliteAttackBehaviour가 없으면 평타만
            meleeAttack.Attack();
            Debug.LogWarning($"[Elite_Orc] {gameObject.name} EliteAttackBehaviour 없음 - 평타로 fallback");
        }
    }

    #endregion

    #region ⭐ Elite_Orc 전용 설정

    /// <summary>
    /// Elite_Orc 전용 설정 적용
    /// </summary>
    private void ApplyEliteOrcSpecificSettings()
    {
        if (EnemyData != null)
        {
            Debug.Log($"[Elite_Orc] {gameObject.name} 전용 설정 적용:");
            ...
            Debug.Log($"  - 특성: 엘리트 오크, 평타 60% + 스킬 40%");
    ...
    private void VerifyEliteType()
    {
        if (EnemyData != null)
        {
            Debug.Log($"[Elite_Orc] 타입 확인: {EnemyData.EnemyType}");
            Debug.Log($"[Elite_Orc] Elite 체력: {GetScaledMaxHealth():F1}");
            Debug.Log($"[Elite_Orc] 평타 확률: {EnemyData.MeleeAttackProbability}%");
            Debug.Log($"[Elite_Orc] 스킬 확률: {EnemyData.SkillUseProbability}%");
        }
    }
    ...
    private void TestMeleeDamage()
    {
        if (meleeAttack != null)
        {
            ...
            Debug.Log($"[Elite_Orc] 실제 근접 데미지: {actualDamage}");
        }
        else
        {
            Debug.LogError($"[Elite_Orc] MeleeAttack 컴포넌트가 없어서 데미지 테스트 불가!");
        }
    }
    ...
    private void CheckSkillCooldowns()
    {
        ...
        {
            Debug.LogWarning($"[Elite_Orc] EliteSkillController가 없습니다!");
        }
    }
    ...
    private void CheckAttackStatistics()
    {
        ...
        {
            Debug.LogWarning($"[Elite_Orc] EliteAttackBehaviour가 없습니다!");
        }
    }
    ...
    public override void LevelUp()
    {
        ...
        Debug.Log($"[Elite_Orc] {gameObject.name} 레벨업! 새 레벨: {CurrentLevel}");
        Debug.Log($"[Elite_Orc] 새 체력: {GetScaledMaxHealth():F1}");
        Debug.Log($"[Elite_Orc] 공격력은 AttackData에서 관리됨");
    }
    ...
    /// Elite_Orc 상태 디버그 출력
    /// </summary>
    [ContextMenu("Debug Elite_Orc Info")]
    private void DebugEliteOrcInfo()
    {
        string info = $"=== Elite_Orc {gameObject.name} ===\n";
    ...
    private void OnValidate()
    {
        ...
        {
            Debug.LogWarning($"[Elite_Orc] {name}의 아이소메트릭 데이터가 유효하지 않습니다.");
            isometricData.SetDefaults();
        }
    }
