...
/// <summary>
/// Boss_SandElemental 보스 몬스터 클래스 - 근접 공격 + 4개 스킬 + 페이즈 전환 (8방향 스프라이트)
/// ⭐ 보스 전용: 평타 + 페이즈별 스킬 시스템 + HP 기반 페이즈 전환
/// ⭐ 완전한 데이터 기반 시스템
/// </summary>
public class Boss_SandElemental : BaseEnemy
{
    [Header("⭐ Boss_SandElemental 전용 컴포넌트")]
    ...
    { 
        get 
        {
            ...
            Debug.LogError($"[Boss_SandElemental] {gameObject.name}: MeleeAttack 또는 AttackData가 없습니다!");
    ...
    public override float DetectionRange
    {
        get
        {
            if (EnemyData != null)
            {
                ...
                Debug.Log($"[Boss_SandElemental] {gameObject.name} DetectionRange: {range} (from EnemyData)");
                return range;
            }
                
            Debug.LogError($"[Boss_SandElemental] {gameObject.name}: EnemyData가 없어서 DetectionRange 확인 불가! fallback 8f 사용");
    ...
    { 
        get 
        {
            if (EnemyData != null)
            {
                ...
                Debug.Log($"[Boss_SandElemental] {gameObject.name} ChaseRange: {range} (from EnemyData)");
                return range;
            }
                
            Debug.LogError($"[Boss_SandElemental] {gameObject.name}: EnemyData가 없어서 ChaseRange 확인 불가! fallback 12f 사용");
    ...
    protected override void OnAwakeInitialize()
    {
        ...
        Debug.Log($"[Boss_SandElemental] {gameObject.name} Awake 초기화 완료");
    }
    ...
    protected override void InitializeAttackSystem()
    {
        ...
        if (meleeAttack != null)
        {
            ...
            Debug.Log($"[Boss_SandElemental] {gameObject.name} MeleeAttack 시스템 초기화 완료");
        }
        else
        {
            Debug.LogError($"[Boss_SandElemental] {gameObject.name}: MeleeAttack 컴포넌트가 없습니다!");
        }
        ...
        if (bossAttack != null)
        {
            ...
            Debug.Log($"[Boss_SandElemental] {gameObject.name} BossAttackBehaviour 시스템 초기화 완료");
        }
        else
        {
            Debug.LogError($"[Boss_SandElemental] {gameObject.name}: BossAttackBehaviour 컴포넌트가 없습니다!");
        }
    }
    ...
    public override void Attack()
    {
        ...
        {
            bossAttack.Attack();
            Debug.Log($"[Boss_SandElemental] {gameObject.name} 보스 공격 실행 (평타 or 스킬)!");
        }
        else if (meleeAttack != null && meleeAttack.CanAttack())
        {
            // Fallback: BossAttackBehaviour가 없으면 평타만
            meleeAttack.Attack();
            Debug.LogWarning($"[Boss_SandElemental] {gameObject.name} BossAttackBehaviour 없음 - 평타로 fallback");
        }
    }
    
    #endregion
    
    #region ⭐ Boss_SandElemental 전용 설정
    
    /// <summary>
    /// Boss_SandElemental 전용 설정 적용
    /// </summary>
    private void ApplyBossSandElementalSpecificSettings()
    {
        if (EnemyData != null)
        {
            Debug.Log($"[Boss_SandElemental] {gameObject.name} 보스 전용 설정 적용:");
        ...
        if (phaseController != null)
        {
            Debug.Log($"[Boss_SandElemental] 페이즈 시스템 활성화: {phaseController.TotalPhases}개 페이즈");
        }
    }
    ...
    public override IsometricCharacterData GetIsometricData()
    {
        ...
        {
            Debug.LogWarning($"[Boss_SandElemental] {gameObject.name}: IsometricData가 없어서 기본값 생성");
    ...
    private void OnValidate()
    {
        ...
        {
            Debug.LogWarning($"[Boss_SandElemental] {name}의 아이소메트릭 데이터가 유효하지 않습니다.");
            isometricData.SetDefaults();
        }
    }
    ...
    public void DebugBossPhaseInfo()
    {
        ...
        {
            Debug.LogWarning("[Boss_SandElemental] PhaseController가 없습니다!");
        }
    }
